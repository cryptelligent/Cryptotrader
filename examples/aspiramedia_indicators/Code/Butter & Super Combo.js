# SUPER & BUTTERWORTH FILTER COMBO (2-POLE/3-POLE)
# DERIVED FROM 'Cybernetic Analysis for Stocks and Futures'
# 14/07/2014

class BUTTERWORTH_TWO_POLE
    constructor: (@period) ->
        @price_array = []
        @smooth_array = []
        @count = 0
    ...
    calculate: (instrument) ->
        ...
        if @count < 3
            ...
            @smooth_array[0] = d*(@price_array[0]+2*@price_array[1]+@price_array[3])+b*@smooth_array[1]+c*@smooth_array[2]
        
        # RETURN SMOOTHED DATA
        return @smooth_array[0]

class BUTTERWORTH_THREE_POLE
    constructor: (@period) ->
        @price_array = []
        @smooth_array = []
        @count = 0
    ...
    calculate: (instrument) ->
        ...
        if @count < 4
            ...
            @smooth_array[0] = g*(@price_array[0]+3*@price_array[1]+3*@price_array[2]+@price_array[3])+d*@smooth_array[1]+e*@smooth_array[2]+f*@smooth_array[3]
...
init: (context)->
    ...
    # THREE-POLE OBJECTS
    context.buttertwo = new BUTTERWORTH_TWO_POLE(15)
    context.butterthree = new BUTTERWORTH_THREE_POLE(15)
...
handle: (context, data)->
    ...
    buttertwo = context.buttertwo.calculate(instrument)
    butterthree = context.butterthree.calculate(instrument)
    supertwo = context.two.calculate(instrument)
    superthree = context.three.calculate(instrument)

    plot
        buttertwo: buttertwo
        butterthree: butterthree
        supertwo: supertwo
