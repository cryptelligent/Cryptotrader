...
#    @accbands: (high, low, close, lag, period) ->
    ...
    # 50/50 balance in very special ocasions
    if (trade == 0) # only trades if stop loss and acc were not actvated
        if (assetsAvailable > currencyAvailable/primaryInstrument.price and short > storage.meanBuy) # SELL
            SellAmount = Math.max(Math.min(_maximumOrderAmount, maximumSellAmount/_iceberg), MINIMUM_AMOUNT)
            if (_plotGraphs)
                plotMark
                    sellIndicator: short
            if (maximumSellAmount >= MINIMUM_AMOUNT)
                ...
                trade = 1
                warn "sell"
        else if (assetsAvailable < currencyAvailable/primaryInstrument.price and short < storage.meanSell) # BUY
            BuyAmount = Math.max(Math.min(_maximumOrderAmount, maximumBuyAmount/_iceberg), MINIMUM_AMOUNT)
            if (_plotGraphs)
                plotMark
                    buyIndicator: short
            if (maximumBuyAmount >= MINIMUM_AMOUNT)
